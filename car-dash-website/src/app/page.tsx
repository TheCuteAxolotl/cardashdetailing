      ...
      <main className="mx-auto max-w-7xl px-6 pb-20 pt-14">
        <section className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div className="max-w-2xl">
            <p className="mb-6 inline-flex rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium uppercase tracking-[0.3em] text-red-900">
          ...
          <div className="rounded-3xl border border-slate-200 bg-slate-50 p-8 shadow-sm shadow-slate-100/80">
            <div className="rounded-3xl bg-white p-8 shadow-inner shadow-slate-100">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.3em] text-red-900">Why choose us</p>
                  <p className="mt-2 text-2xl font-semibold text-slate-950">Focused on detail, not drama.</p>
                </div>
                <div className="rounded-2xl bg-red-950 px-3 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-white">
                  Clean luxury
                </div>
              </div>
              <div className="space-y-4">
                <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                  <p className="text-sm font-semibold text-slate-950">Precise results</p>
                  <p className="mt-2 text-sm leading-6 text-slate-600">Every detail is handled with care so your vehicle looks its best without overstatement.</p>
                </div>
                <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                  <p className="text-sm font-semibold text-slate-950">Premium service</p>
                  <p className="mt-2 text-sm leading-6 text-slate-600">Professional guidance and clean execution for a confident finish.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <a
              key={item.title}
              href={item.href}
              className="rounded-3xl border border-slate-200 bg-white p-6 text-slate-950 transition hover:-translate-y-1 hover:border-red-900 hover:bg-slate-50"
            >
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-red-900">{item.title}</p>
