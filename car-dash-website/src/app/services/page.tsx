    ...
    <main className="min-h-screen bg-[#0D0D0D] text-white">
      <section className="relative overflow-hidden border-b border-white/10 px-6 py-24 text-center">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          <a href="/estimate" className="rounded-full bg-[#FF2D2D] px-6 py-3 font-semibold text-[#0D0D0D] shadow-[0_0_32px_rgba(255,45,45,.12)]">Get an Estimate</a>
      ...
      <div className="mx-auto max-w-7xl px-6 py-14">
        {categories.length ? categories.map((category) => (
          <section key={category} className="mb-14">
            <p className="text-xs font-bold uppercase tracking-[.28em] text-[#FF2D2D]">{category}</p>
            <div className="mt-5 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {items.filter((item) => item.category === category).map((service) => (
                <article id={service.id} key={service.id} className="group rounded-[28px] border border-white/10 bg-[linear-gradient(145deg,rgba(74,85,104,.12),rgba(255,255,255,.02))] p-6 transition duration-300 hover:-translate-y-1 hover:border-[#FF2D2D]/35 hover:bg-[#FF2D2D]/[.035]">
