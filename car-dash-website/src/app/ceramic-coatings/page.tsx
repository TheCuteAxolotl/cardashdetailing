    ...
    <main className="min-h-screen bg-[#F4F3EF] text-[#111]">
      <section className="border-b border-black/8 bg-white">
        <div className="mx-auto grid max-w-7xl gap-6 px-5 py-14 sm:px-8 sm:py-20 lg:grid-cols-[.9fr_1.1fr] lg:items-end">
          <div><p className="text-xs font-bold uppercase tracking-[.22em] text-[#FF2D2D]">Ceramic coating</p><h1 className="mt-3 text-5xl font-semibold leading-[.94] tracking-[-.06em] sm:text-7xl">Good coating starts with good prep.</h1></div>
          <div><p className="max-w-2xl text-base leading-7 text-black/52">We use GYEON and Gtechniq systems. The paint is washed, decontaminated, and corrected as needed before the coating goes on.</p><div className="mt-5 flex flex-wrap gap-3"><a href="/#specialty-prices" className="rounded-full bg-[#111] px-5 py-3 text-sm font-bold text-white">See current prices</a><a href="/quote" className="rounded-full bg-[#FF2D2D] px-5 py-3 text-sm font-bold text-white">Get exact quote</a><a href="/#book" className="rounded-full border border-black/12 px-5 py-3 text-sm font-semibold">Book</a></div></div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-12 sm:px-8 sm:py-16">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2">
          {systems.map(([name, tag, body]) => <article key={name} className="rounded-[24px] border border-black/10 bg-white p-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#FF2D2D]">{tag}</p><h2 className="mt-3 text-2xl font-semibold tracking-[-.035em]">{name}</h2><p className="mt-3 text-sm leading-6 text-black/48">{body}</p></article>)}
        </div>
      </section>
      ...
      <section className="border-y border-black/8 bg-[#111] text-white">
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 sm:px-8 lg:grid-cols-2">
          <div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#FF2D2D]">What it does</p><h2 className="mt-3 text-3xl font-semibold tracking-[-.04em]">More gloss, easier washing, better water behavior, and longer-term paint protection.</h2></div>
