    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#07131B] px-5 text-white">
        <div className="max-w-md rounded-3xl border border-white/10 bg-white/[.025] p-8 text-center">
          ...
          <a href="/login" className="mt-6 inline-block rounded-full bg-[#6EAEC6] px-5 py-3 text-sm font-semibold text-[#0B1822]">Sign in</a>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#07131B] px-5 py-8 text-white sm:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[.26em] text-[#6EAEC6]">Booking conversation</p>
          ...
          <section className="flex min-h-[650px] flex-col rounded-[28px] border border-white/10 bg-white/[.025] p-5 sm:p-6">
              ...
              {data.messages.length ? (
                data.messages.map((message) => {
                  ...
                  return (
                    <div
                      key={message.id}
                      className={`max-w-[86%] rounded-2xl px-4 py-3 ${mine ? "ml-auto bg-[#6EAEC6] text-[#0B1822]" : "bg-white/8"}`}
                    >
            ...
            <form onSubmit={send} className="flex gap-2 border-t border-white/10 pt-4">
              ...
              <button
                ...
                className="self-end rounded-full bg-[#6EAEC6] px-5 py-3 text-sm font-semibold text-[#0B1822] disabled:cursor-not-allowed disabled:opacity-50"
              >
