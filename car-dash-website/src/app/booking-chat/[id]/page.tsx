    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#050505] px-5 text-white">
        <div className="max-w-md rounded-3xl border border-white/10 bg-white/[.025] p-8 text-center">
          <p className="text-xs font-semibold uppercase tracking-[.24em] text-red-400">Booking chat</p>
          ...
          <a href="/login" className="mt-6 inline-block rounded-full bg-red-600 px-5 py-3 text-sm font-semibold">Sign in</a>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#050505] px-5 py-8 text-white sm:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[.26em] text-red-400">Booking conversation</p>
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-[1fr_320px]">
          <section className="flex min-h-[650px] flex-col rounded-[28px] border border-white/10 bg-white/[.025] p-5 sm:p-6">
              ...
              {data.messages.length ? (
                data.messages.map((message) => {
                  ...
                  return (
                    <div
                      key={message.id}
                      className={`max-w-[86%] rounded-2xl px-4 py-3 ${mine ? "ml-auto bg-red-600" : "bg-white/8"}`}
                    >
            ...
            <form onSubmit={send} className="flex gap-2 border-t border-white/10 pt-4">
              <textarea
                ...
                maxLength={3000}
                className="min-h-12 flex-1 resize-none rounded-2xl border border-white/10 bg-black/50 px-4 py-3 text-sm text-white outline-none focus:border-red-500/50"
              />
              <button
                disabled={sending || !reply.trim()}
                className="self-end rounded-full bg-red-600 px-5 py-3 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50"
              >
          ...
          <aside className="h-fit rounded-[28px] border border-white/10 bg-white/[.025] p-5">
            <p className="text-xs font-semibold uppercase tracking-[.22em] text-red-400">Booking details</p>
