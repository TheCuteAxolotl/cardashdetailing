...
const notifyNumbers =
  process.env.TWILIO_NOTIFY_NUMBERS
    ?.split(",")
    .map((num) => num.trim())
    .filter(Boolean) ?? [];
...
async function sendSmsNotification(body: string) {
  if (!twilioClient || !fromNumber || !notifyNumbers.length) {
    console.log(
      "Booking request received without Twilio configured:",
      body
    );
    return false;
  }
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Invalid form submission format. Booking must be sent as form data.",
        },
        { status: 415 }
      );
    }
    ...
    const name = getRequired(
      formData.get("name"),
      "name"
    );

    const phone = getRequired(
      formData.get("phone"),
      "phone"
    );

    const email = getRequired(
      formData.get("email"),
      "email"
    );

    const vehicleMake = getRequired(
      formData.get("vehicleMake"),
      "vehicleMake"
    );

    const vehicleModel = getRequired(
      formData.get("vehicleModel"),
      "vehicleModel"
    );

    const vehicleYear = getRequired(
      formData.get("vehicleYear"),
      "vehicleYear"
    );

    const vehicleTrim = String(
      formData.get("vehicleTrim") ?? ""
    );
    ...
    const serviceMethod = String(
      formData.get("serviceMethod") ?? "mobile"
    );
