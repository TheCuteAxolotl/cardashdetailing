...
export async function GET(request: NextRequest) {
  try {
    ...
    const bookings = await prisma.booking.findMany({ where: staff ? {} : { userId: auth.id }, include: { user: { select: { name: true, email: true } } }, orderBy: { createdAt: "desc" } });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const vehicleId=String(form.get("vehicleId")||"").trim()||null;
    const booking=await prisma.booking.create({ data:{ userId:auth?.id??null, vehicleId, serviceName, serviceMethod, customerName:name, customerEmail:email, customerPhone:phone, vehicleMake, vehicleModel, vehicleYear, vehicleTrim, preferredDate, preferredTime: preferredTime || null, notes:details } });
