...
export async function GET() {
  try {
    ...
    const access = await getPermissionSnapshotForUser(user);

    return NextResponse.json(
      {
        user: { ...user, role: access.role },
    ...
    if (isLikelyDatabaseError(error)) {
      return NextResponse.json({ error: databaseUnavailableResponseMessage() }, { status: 503 });
    }
