...
export default function QuotePage() {
  ...
  if (user === undefined) return <main className="min-h-screen bg-[#070707] p-10 text-white">Loading…</main>;

  if (!user) {
    return (
      <main className="min-h-screen bg-[#070707] px-6 py-24 text-white">
        <div className="mx-auto max-w-xl text-center">
          <p className="text-red-400">Specialist Chat</p>
          ...
          <div className="mt-7 flex justify-center gap-3">
            <a href="/login" className="rounded-full bg-red-600 px-6 py-3">Login</a>
  ...
  return (
    <main className="min-h-screen bg-[#070707] px-4 py-8 text-white">
      <div className="mx-auto max-w-7xl">
        <div>
          <p className="text-xs uppercase tracking-[.28em] text-red-400">Private quote chat</p>
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-[300px_1fr]">
          <aside className="rounded-[26px] border border-white/10 bg-white/[.025] p-3">
            <button onClick={() => setSelected("")} className="mb-3 w-full rounded-2xl bg-red-600 px-4 py-3 text-left font-semibold">
              + New quote chat
            </button>
            {threads.map((thread) => (
              <button
            ...
            {active ? (
                ...
                <div className="border-b border-white/10 pb-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    {active.status === "accepted" && active.quotedPrice && (
                      <a href={`/contact?quote=${encodeURIComponent(active.id)}`} className="rounded-full bg-red-600 px-5 py-2.5 text-sm font-semibold">
                  ...
                  {active.quotedPrice && (
                    <div className="mt-4 rounded-2xl border border-red-500/20 bg-red-500/[.06] p-4">
                      <p className="text-xs uppercase tracking-[.2em] text-red-400">Car Dash Final Quote</p>
                      ...
                      {active.status === "quoted" && (
                        <button onClick={accept} className="mt-3 rounded-full bg-red-600 px-5 py-2 text-sm font-semibold">Accept quote</button>
                      )}
                ...
                <div className="flex-1 space-y-3 overflow-y-auto py-5">
                  {active.messages.map((message) => {
                    ...
                    return (
                      <div
                        key={message.id}
                        className={`max-w-[82%] rounded-2xl px-4 py-3 ${message.sender === "customer" ? "ml-auto bg-white/10" : "bg-red-600"}`}
                      >
                ...
                {readOnly ? (
                  ...
                  <form onSubmit={send} className="border-t border-white/10 pt-4">
                    <div className="flex gap-2">
                      ...
                      <button className="rounded-full bg-red-600 px-5 font-semibold">Send</button>
                    </div>
                    ...
                    {photos.length > 0 && <span className="ml-3 text-xs text-red-300">{photos.length} photo(s) ready</span>}
                  </form>
                )}
              ...
              <form onSubmit={create} className="mx-auto max-w-2xl py-4">
                ...
                <div className="mt-5 grid gap-4">
                  ...
                  {!vehicles.length && <a href="/vehicles" className="text-sm text-red-400">+ Save a vehicle first</a>}
                  ...
                  {photos.length > 0 && <p className="text-sm text-red-300">{photos.length} photo(s) selected</p>}
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/[.025] p-4">
                    ...
                    <p className="mt-3 pl-6 text-xs text-white/35">
                      See our <a href="/privacy-policy" className="text-red-400">Privacy Policy</a> and{" "}
                      <a href="/terms-and-conditions" className="text-red-400">Terms and Conditions</a>.
                    </p>
                  </div>

                  {error && <p className="text-sm text-red-300">{error}</p>}
                  <button className="rounded-full bg-red-600 px-6 py-3 font-semibold">Start private chat</button>
                </div>
              </form>
            )}
          </section>
