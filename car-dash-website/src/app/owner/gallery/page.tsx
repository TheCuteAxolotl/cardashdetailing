    ...
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <main className="mx-auto max-w-7xl px-6 py-10">
        <form onSubmit={addImage} className="grid gap-4 rounded-3xl border border-neutral-800 bg-neutral-950 p-6 md:grid-cols-4">
          ...
          <div><label className="mb-2 block text-sm font-medium">Placement</label><select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full rounded-xl border border-neutral-800 bg-neutral-900 p-3 text-sm"><option value="gallery">Public Gallery</option><option value="before-after">Before & After</option><option value="portfolio">Portfolio</option><option value="hero">Homepage Hero</option><option value="home-showcase-primary">Homepage Showcase — Large</option><option value="home-showcase-secondary">Homepage Showcase — Small</option><option value="home-story">Homepage Story</option><option value="home-process">Homepage Process</option><option value="services-hero">Services Page Hero</option><option value="gallery-hero">Gallery Page Hero</option><option value="reviews-hero">Reviews Page Hero</option><option value="contact-hero">Contact Page Hero</option></select></div>
            ...
            {images.map((image) => (
              <article key={image.id} className="overflow-hidden rounded-3xl border border-neutral-800 bg-neutral-950">
                ...
                <div className="space-y-3 p-4">
                  ...
                  <select value={image.category} onChange={(e) => updateImage(image, { category: e.target.value }).catch(() => setMessage("Update failed."))} className="w-full rounded-lg border border-neutral-800 bg-neutral-900 px-3 py-2 text-sm"><option value="gallery">Public Gallery</option><option value="before-after">Before & After</option><option value="portfolio">Portfolio</option><option value="hero">Homepage Hero</option><option value="home-showcase-primary">Homepage Showcase — Large</option><option value="home-showcase-secondary">Homepage Showcase — Small</option><option value="home-story">Homepage Story</option><option value="home-process">Homepage Process</option><option value="services-hero">Services Page Hero</option><option value="gallery-hero">Gallery Page Hero</option><option value="reviews-hero">Reviews Page Hero</option><option value="contact-hero">Contact Page Hero</option></select>
