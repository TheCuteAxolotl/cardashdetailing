    ...
    <div className="min-h-screen bg-[#050505] text-white">
      <header className="border-b border-neutral-800 bg-neutral-950">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
          ...
          <a href="/owner/dashboard" className="rounded-lg bg-[#FF2D2D] px-4 py-2 text-sm font-medium text-[#0D0D0D] hover:bg-[#FF2D2D]">Back</a>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl px-6 py-10">
        <form onSubmit={addImage} className="grid gap-4 rounded-3xl border border-neutral-800 bg-neutral-950 p-6 md:grid-cols-4">
          ...
          <div className="md:col-span-4"><button disabled={saving} className="rounded-xl bg-[#FF2D2D] px-6 py-3 font-semibold text-[#0D0D0D] hover:bg-[#FF2D2D] disabled:opacity-50">{saving ? "Processing photo…" : "Add Photo"}</button>{message && <span className="ml-4 text-sm text-neutral-300">{message}</span>}</div>
        </form>
            ...
            {images.map((image) => (
              <article key={image.id} className="overflow-hidden rounded-3xl border border-neutral-800 bg-neutral-950">
                ...
                <div className="space-y-3 p-4">
                  ...
                  <select value={image.category} onChange={(e) => updateImage(image, { category: e.target.value }).catch(() => setMessage("Update failed."))} className="w-full rounded-lg border border-neutral-800 bg-neutral-900 px-3 py-2 text-sm"><option value="gallery">Public Gallery</option><option value="before-after">Before & After</option><option value="portfolio">Portfolio</option><option value="hero">Homepage — Main Background</option><option value="home-showcase-primary">Homepage — Showcase Large</option><option value="home-showcase-secondary">Homepage — Showcase Small</option><option value="home-story">Homepage — Story Photo</option><option value="home-services-bg">Homepage — Services Background</option><option value="home-cta-bg">Homepage — Booking Background</option><option value="services-hero">Services — Background</option><option value="gallery-hero">Gallery — Background</option><option value="reviews-hero">Reviews — Background</option><option value="contact-hero">Contact — Background</option><option value="about-hero">About — Background</option><option value="about-story">About — Story Photo</option><option value="about-values-bg">About — Values Background</option><option value="faq-hero">FAQ — Background</option></select>
