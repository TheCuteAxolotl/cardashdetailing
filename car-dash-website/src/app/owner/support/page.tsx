...
export default function OwnerSupportPage() {
  ...
  const block = async (event: FormEvent) => {
    ...
    if (!identifier.trim()) {
      setMessage("Enter an IP address or visitor hash.");
      return;
    }
    ...
    setMessage(
      data.message ||
        data.error ||
        (response.ok ? "Visitor blocked." : "Unable to block visitor.")
    );
  ...
  const unblock = async () => {
    ...
    setMessage(
      data.message ||
        data.error ||
        (response.ok ? "Visitor unblocked." : "Unable to unblock visitor.")
    );
    ...
    <main className="min-h-screen bg-[#080808] text-white">
      <div className="mx-auto max-w-[1500px] px-5 py-8 sm:px-8 lg:px-12">
        <div className="flex flex-wrap items-end justify-between gap-4 border-b border-white/10 pb-6">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[.28em] text-red-400">
              Owner
            </p>
            ...
            <p className="mt-2 text-sm text-white/38">
              Reply to website support chats and manage repeat spam using the
              identifier from Discord.
            </p>
          </div>
          ...
          <section className="min-h-[620px] rounded-[24px] border border-white/10 bg-white/[.025] p-5 sm:p-7">
            {selected ? (
              <>
                ...
                <form
                  onSubmit={sendReply}
                  className="mt-auto flex gap-2 border-t border-white/8 pt-5"
                >
                  <input
                    value={reply}
                  ...
                  <button className="rounded-full bg-white px-5 text-sm font-semibold text-black transition hover:bg-white/85">
                    Send
                  </button>
                </form>
              </>
        ...
        <section className="rounded-[24px] border border-red-500/20 bg-red-500/[.04] p-6">
          <p className="text-[10px] font-semibold uppercase tracking-[.25em] text-red-400">
            Spam controls
          </p>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-white/38">
            Paste the IP address or visitor hash shown in the Discord support
            alert. The website does not display these identifiers anywhere.
          </p>
          ...
          >
            <input
              ...
              placeholder="IP or visitor hash from Discord"
              className="support-input"
              required
            />
            ...
            <button
              type="submit"
              className="rounded-full bg-red-600 px-6 py-3 text-sm font-semibold transition hover:bg-red-500"
            >
              Block
            </button>
            ...
            <button
              type="button"
              onClick={unblock}
              className="rounded-full border border-white/15 px-6 py-3 text-sm font-semibold text-white/70 transition hover:border-white/35 hover:bg-white/5 hover:text-white"
            >
              Unblock
            </button>
          </form>
          ...
          {message && (
            <p className="mt-4 rounded-2xl border border-white/8 bg-white/[.025] px-4 py-3 text-sm text-white/60">
              {message}
            </p>
          )}
        </section>
