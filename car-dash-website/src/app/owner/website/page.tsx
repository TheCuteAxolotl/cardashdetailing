    ...
    <div className="min-h-screen bg-[#070707] text-white">
      <header className="border-b border-white/10 bg-black/40">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-5 px-6 py-6">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.28em] text-[#FF2D2D]">Owner</p>
        ...
        {loading ? <p className="text-white/45">Loading website content…</p> : (
          <div className="space-y-6">
                ...
                <div className="mt-5 grid gap-5 md:grid-cols-2">
                  {fields.filter((field) => field.group === group).map((field) => (
                    <label key={field.key} className={field.area ? "md:col-span-2" : ""}>
                      ...
                      {field.area ? (
                        <textarea value={content[field.key]} onChange={(e) => setContent({ ...content, [field.key]: e.target.value })} rows={3} className="w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm leading-6 text-white outline-none focus:border-[#FF2D2D]/60" />
                      ) : (
                        <input value={content[field.key]} onChange={(e) => setContent({ ...content, [field.key]: e.target.value })} className="w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none focus:border-[#FF2D2D]/60" />
                      )}
                    </label>
                  ))}
                </div>
            ...
            <div className="sticky bottom-5 flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-[#111]/95 p-4 shadow-2xl backdrop-blur">
              ...
              <button onClick={save} disabled={saving} className="rounded-full bg-[#FF2D2D] px-6 py-3 text-sm font-semibold text-[#0D0D0D] transition hover:bg-[#FF2D2D] disabled:opacity-50">{saving ? "Saving…" : "Save Website"}</button>
            </div>
          </div>
        )}
      </main>
