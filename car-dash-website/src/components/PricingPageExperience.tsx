    ...
    <main className="min-h-screen bg-[#EEF7F5] text-[#111]">
      <section className="border-b border-black/8 bg-white">
        <div className="mx-auto grid max-w-7xl gap-7 px-5 py-10 sm:px-8 sm:py-14 lg:grid-cols-[.9fr_1.1fr] lg:items-stretch">
          <div className="flex flex-col justify-center py-3 lg:py-8">
            <p className="text-xs font-bold uppercase tracking-[.22em] text-[#6EAEC6]">{pageLabels[kind]}</p>
      ...
      <section id="pricing" className="mx-auto max-w-7xl scroll-mt-28 px-5 py-12 sm:px-8 sm:py-16">
        <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.22em] text-[#6EAEC6]">Standard prices</p>
              ...
              <div className="mt-5 overflow-hidden rounded-2xl border border-black/10 bg-[#F7F7F5]">
                {(Object.keys(VEHICLE_LABELS) as VehicleClass[]).map((vehicleClass, index) => (
                  <a key={vehicleClass} href={bookingHref(kind, pkg.id, vehicleClass)} className={`flex min-h-14 items-center justify-between gap-4 px-4 py-3 hover:bg-white ${index ? "border-t border-black/8" : ""}`}>
                    ...
                    <span className="flex items-center gap-3"><strong className="text-lg">${Number(pkg.prices[vehicleClass] || 0).toFixed(0)}</strong><span className="text-xs font-semibold text-[#6EAEC6]">Book →</span></span>
                  </a>
                ))}
              </div>
      ...
      <section className="border-t border-black/8 bg-[#111] text-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-5 py-12 sm:flex-row sm:items-center sm:justify-between sm:px-8">
          <div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#6EAEC6]">Ready?</p><h2 className="mt-2 text-3xl font-semibold tracking-[-.04em]">Choose a price and book an open time.</h2></div>
          <div className="flex gap-3"><a href="/#book" className="rounded-full bg-[#6EAEC6] px-5 py-3 text-sm font-bold">Book now</a><a href="/quote" className="rounded-full border border-white/15 px-5 py-3 text-sm font-semibold">Exact quote</a></div>
        </div>
      </section>
    </main>
  );
