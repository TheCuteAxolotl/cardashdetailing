      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#070707]/94 text-white backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1540px] items-center justify-between gap-5 border-x border-white/10 px-5 py-4 sm:px-8 lg:px-10">
          ...
          <div className="hidden items-center gap-2 xl:flex">
            {!user ? (
              <a href="/login" className="rounded-full border border-white/15 px-4 py-2.5 text-xs font-semibold text-white/65 transition hover:border-white/35 hover:text-white">Login</a>
            ) : (
              <button onClick={logout} className="px-3 text-xs font-semibold text-white/35 transition hover:text-white">Logout</button>
            )}
        ...
        {menuOpen && (
          <div className="border-t border-white/10 bg-[#080808] px-5 py-4 sm:px-8 xl:hidden">
            <nav className="flex flex-col text-sm text-white/60">
              {publicLinks.map(([href, label]) => (
                <a key={href} href={href} className="rounded-xl px-3 py-3 hover:bg-white/5 hover:text-white">{label}</a>
              ))}
              ...
              <a href="/contact" className="mt-2 rounded-full bg-red-600 px-5 py-3 text-center font-semibold text-white">Book Now</a>
            </nav>
          </div>
        )}
      </header>
