    ...
    <>
      <div className="border-b border-red-500/15 bg-red-600 text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-2 text-[10px] font-black uppercase tracking-[0.2em] sm:px-6 sm:text-[11px]">
          ...
          <Link href="/contact" className="hidden underline-offset-4 hover:underline sm:inline">
            Request a Detail →
          </Link>
        </div>
      </div>
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#070707]/95 text-white backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-4 sm:px-6">
          <Link href="/" className="group flex items-center gap-3">
            <span className="grid h-9 w-9 place-items-center rounded-full border border-red-500/35 bg-red-500/10 text-sm font-black text-red-500 transition group-hover:bg-red-600 group-hover:text-white">
              CD
            </span>
            <div>
          ...
          <nav className="hidden items-center gap-7 text-sm font-bold text-neutral-300 md:flex">
            ...
            <Link href="/services" className="transition hover:text-red-400">Services</Link>
            <Link href="/gallery" className="transition hover:text-red-400">Gallery</Link>
            <Link href="/reviews" className="transition hover:text-red-400">Reviews</Link>
            <Link href="/contact" className="transition hover:text-red-400">Contact</Link>
            {owner && (
              <Link href="/owner/dashboard" className="font-black text-red-400 transition hover:text-red-300">
                Owner Dashboard
              </Link>
            )}
          </nav>
          ...
          <div className="hidden items-center gap-3 md:flex">
            {owner && (
              <button onClick={logout} className="text-xs font-bold text-neutral-500 transition hover:text-white">
                Logout
              </button>
            )}
            <Link
        ...
        {menuOpen && (
          <div className="border-t border-white/10 bg-[#070707] px-5 pb-6 sm:px-6 md:hidden">
            <nav className="flex flex-col gap-1 py-4 text-sm font-bold text-neutral-300">
              ...
              <Link href="/contact" onClick={() => setMenuOpen(false)} className="rounded-xl px-3 py-3 hover:bg-white/5">Contact</Link>
