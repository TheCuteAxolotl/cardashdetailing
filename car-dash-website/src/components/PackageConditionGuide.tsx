        ...
        <div className="mt-3 border-t border-black/8 pt-3">
          ...
          {media.length > 0 && (
            <div className="mt-3">
              ...
              {media.length > 1 && <button type="button" onClick={() => setOpenIndex(0)} className="mt-2 text-xs font-semibold text-[#FF2D2D]">View {media.length} examples →</button>}
            </div>
          )}
          <div className="mt-4 rounded-xl bg-black/[.035] p-3">
            ...
            <a href={quoteHref} className="mt-1 inline-flex text-xs font-bold text-[#FF2D2D]">Get a free photo quote →</a>
          </div>
        </div>
      </details>
