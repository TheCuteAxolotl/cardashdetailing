...
const input = "mt-2 w-full rounded-2xl border border-white/10 bg-black/50 px-4 py-3 text-white outline-none focus:border-red-500/50";
...
export default function BookingForm({
  ...
  return (
    <form onSubmit={submit} className="space-y-5 text-white">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[.24em] text-red-400">
      ...
      {setupMessage && (
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/[.07] p-4 text-sm text-amber-100">
          <p>{setupMessage}</p>
          <a href="/quote" className="mt-3 inline-block font-semibold text-red-300 hover:text-red-200">
      ...
      {quoteLocked ? (
        <div className="rounded-3xl border border-red-500/25 bg-red-500/[.06] p-5">
          <p className="text-xs uppercase tracking-[.22em] text-red-400">Locked booking total</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-sm text-white/60">
          ...
          {form.preferredDate && slots.length === 0 && (
            <span className="mt-2 block text-xs text-red-300">No standard slots available. Try another date or chat with a specialist.</span>
          )}
        </label>
      </div>
      ...
      <label className="flex items-start gap-3 text-sm text-white/55">
        ...
        <span>
          ...
          <a href="/terms-and-conditions" target="_blank" rel="noreferrer" className="text-red-400 hover:text-red-300">Terms and Conditions</a>{" "}
          and acknowledge the{" "}
          <a href="/privacy-policy" target="_blank" rel="noreferrer" className="text-red-400 hover:text-red-300">Privacy Policy</a>.
        </span>
      </label>
      ...
      <div className="flex gap-3">
        <button
          ...
          className="rounded-full bg-red-600 px-6 py-3 font-semibold disabled:cursor-not-allowed disabled:opacity-50"
        >
