...
const input = "w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 outline-none focus:border-[#6EAEC6]/55";
    ...
    <main className="min-h-screen bg-[#070707] px-5 py-8 text-white">
      <div className="mx-auto max-w-[1500px]">
        <div className="flex justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[.28em] text-[#6EAEC6]">Sales inbox</p>
                ...
                <div>
                      ...
                      <div>
                        ...
                        {!active.user && (active.guestPhone || active.guestEmail) && (
                          <div className="mt-3 flex flex-wrap gap-2">
                            {active.guestPhone && <a href={`tel:${active.guestPhone}`} className="rounded-full border border-white/12 px-3 py-1.5 text-xs text-white/60 hover:border-[#6EAEC6]/40 hover:text-white">Call</a>}
                            {active.guestPhone && <a href={`sms:${active.guestPhone}`} className="rounded-full border border-white/12 px-3 py-1.5 text-xs text-white/60 hover:border-[#6EAEC6]/40 hover:text-white">Text</a>}
                            {active.guestEmail && <a href={`mailto:${active.guestEmail}`} className="rounded-full border border-white/12 px-3 py-1.5 text-xs text-white/60 hover:border-[#6EAEC6]/40 hover:text-white">Email</a>}
                          </div>
                        )}
                      </div>
                  ...
                  <div className="space-y-3 py-5">
                    {active.messages.map((message) => {
                      ...
                      return (
                        <div key={message.id} className={`max-w-[85%] rounded-2xl px-4 py-3 ${message.sender === "team" ? "ml-auto bg-[#6EAEC6] text-[#0B1822]" : "bg-white/8"}`}>
                  ...
                  {readOnly ? (
                    ...
                    <form onSubmit={send} className="flex gap-2 border-t border-white/10 pt-4">
                      ...
                      <button className="rounded-full bg-[#6EAEC6] px-5 text-[#0B1822]">Send</button>
                    </form>
                  )}
                </div>

                <aside className="rounded-2xl border border-white/10 bg-black/30 p-4">
                  <p className="text-xs uppercase tracking-[.22em] text-[#6EAEC6]">Final quote</p>
                  ...
                  <button
                    ...
                    className="mt-4 w-full rounded-full bg-[#6EAEC6] px-5 py-3 font-semibold text-[#0B1822] disabled:cursor-not-allowed disabled:opacity-40"
                  >
