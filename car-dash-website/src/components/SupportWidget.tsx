...
export default function SupportWidget() {
  ...
  const loadTicket = async (active: Session) => {
    const response = await fetch(`/api/support/${active.id}`, {
      cache: "no-store",
      headers: {
        "x-support-key": active.key,
      },
    });
  ...
  const createTicket = async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || "Could not send support request."
        );
      }
  ...
  const sendMessage = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/support/${session.id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "x-support-key": session.key,
          },
          body: JSON.stringify({
            message,
          }),
            ...
            {!session ? (
              ...
              >
                ...
                <p className="text-xs leading-5 text-white/30">
                  For website chat, keep this browser/device so
                  the conversation stays connected.
                </p>
              </form>
            ) : (
              <div className="flex min-h-0 flex-1 flex-col">
                <div className="border-b border-white/8 px-5 py-3 text-xs text-white/40">
